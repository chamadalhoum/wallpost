    ...
    public function notificationfichebyid(Request $request) {
     //   $fiche_id=$request->fiche_id;
        ...
        $nbnotif=0;
                ...
                $fiche= Fiche::join('ficheshistoriques','ficheshistoriques.fiche_id','=','fiches.id')
                ->where('fiches.id',$fiche_id)
                ->select('fiches.primaryPhone','fiches.websiteUrl','fiches.locationName','fiches.OpenInfo_opening_date',
                'fiches.additionalPhones','fiches.address','fiches.email',
                'ficheshistoriques.primaryPhone as primaryPhonepre','fiches.websiteUrl',
                'ficheshistoriques.locationName as locationNamepre','ficheshistoriques.OpenInfo_opening_date as OpenInfo_opening_datepre',
                'ficheshistoriques.additionalPhones as additionalPhonespre','ficheshistoriques.address as addresspre','ficheshistoriques.email as emailpre')->first();
            
                if($fiche->primaryPhone != $fiche->primaryPhonepre){
                    $data['phone']=['new'=>$fiche->primaryPhone,'old'=> $fiche->primaryPhonepre];
                   $nbnotif++;
                }
                if($fiche->websiteUrl != $fiche->websiteUrlpre){
                    $data['websiteUrl']=['new'=>$fiche->websiteUrl,'old'=> $fiche->websiteUrlpre];
                    $nbnotif++;
                }
                if($fiche->locationName != $fiche->locationNamepre){
                    $data['locationName']=['new'=>$fiche->locationName,'old'=> $fiche->locationNamepre];
                
                   $nbnotif++;
                }
                if($fiche->OpenInfo_opening_date != $fiche->OpenInfo_opening_datepre){
                    $data['OpenInfo_opening_date']=['new'=>$fiche->OpenInfo_opening_date,'old'=> $fiche->OpenInfo_opening_datepre];
                    $nbnotif++;
                }
                if($fiche->additionalPhones != $fiche->additionalPhonespre){
                    $data['additionalPhones']=['new'=>$fiche->additionalPhones,'old'=> $fiche->additionalPhonespre];
                    $nbnotif++;
                }
                if($fiche->address != $fiche->addresspre){
                    $data['address']=['new'=>$fiche->address,'old'=> $fiche->addresspre];
                    $nbnotif++;
                }
                if($fiche->email != $fiche->emailpre){
                    $data['email']=['new'=>$fiche->email,'old'=> $fiche->emailpre];
                     $nbnotif++;
                }
                $lienprend=Attribute::join('attributeshistoriques','attributes.id','=','attributeshistoriques.attribute_id')
                ->where('attributes.fiche_id',$fiche_id)
                ->where('attributes.valueType',"URL")
                ->select('attributes.urlValues','attributeshistoriques.urlValues as urlValuespre')
                ->whereColumn("attributes.urlValues",'<>',"attributeshistoriques.urlValues");
                if($lienprend->exists()){

                $detailslienprend=$lienprend->get();
                foreach($detailslienprend as  $prendre){
                    $data['lienprend'][]=['new'=>$prendre->urlValues,'old'=> $prendre->urlValuespre];
                }
                 $nbnotif++;
              
                }
                $fournis=Attribute::join('attributeshistoriques','attributes.id','=','attributeshistoriques.attribute_id')
                ->where('attributes.fiche_id',$fiche_id)
                ->where('attributes.valueType','<>',"URL")
                ->select('attributes.displayName','attributeshistoriques.displayName as displayNamepre')
                ->whereColumn("attributes.attributeId",'<>',"attributeshistoriques.attributeId");
                if($fournis->exists()){
                $detailsfournis=$fournis->get();
                foreach($detailsfournis as  $fourn){
                    $data['fournis'][]=['new'=>$fourn->displayName,'old'=> $fourn->displayNamepre];
                }
                 $nbnotif++;
              
                }
           
                $horaire=Fichehour::join('fiche_hourhistoriques','fichehours.id','=','fiche_hourhistoriques.fichehours_id')
                ->where('fichehours.fiche_id',$fiche_id)
                ->select('fichehours.open_date','fiche_hourhistoriques.open_date as open_datepre')
                ->whereColumn("fichehours.open_time",'<>',"fiche_hourhistoriques.open_time");
                if($horaire->exists()){
                $detailshoraire=$horaire->get();
                foreach($detailshoraire as  $hor){
                    $data['horaire'][]=['datedebutnew'=>$hor->open_date,'old'=> $hor->open_datepre];
                }
                 $nbnotif++;
              
                }
                $service=Service::join('serviceshistoriques','services.id','=','serviceshistoriques.service_id')
                ->join('categories','categories.id','=','services.categorie_id')
                ->select('services.displayName','serviceshistoriques.displayName as displayNamepre')
                ->where('categories.fiche_id',$fiche_id)
         
                ->whereColumn("services.serviceId",'<>',"serviceshistoriques.serviceId");
                if($service->exists()){
                $detailservice=$service->get();
                foreach($detailservice as  $service){
                    $data['service'][]=['new'=>$service->displayName,'old'=> $service->displayNamepre];
                }
                 $nbnotif++;
               }
                $zonedever=Servicearea::join('serviceareashistoriques','serviceareas.id','=','serviceareashistoriques.serviceareas_id')
                ->select('serviceareas.name','serviceareashistoriques.name as namepre')
                ->where('serviceareas.fiche_id',$fiche_id)
                ->whereColumn("serviceareas.placeId",'<>',"serviceareashistoriques.placeId");
                if($zonedever->exists()){
                $detailszone=$zonedever->get();
                foreach($detailszone as  $zone){
                    $data['zonedesservies'][]=['new'=>$zone->name,'old'=> $zone->namepre];
                }
                 $nbnotif++;
              }


$histcat=Categorie::join('categorieshistoriques',"categorieshistoriques.categorie_id",'=','categories.id')
->select('categories.displayName','categorieshistoriques.categorie_id','categorieshistoriques.displayName as displayNameprecedent')
->where('categories.fiche_id',$fiche_id)
->whereColumn("categories.categorieId",'<>',"categorieshistoriques.categorieId");

              if($histcat->exists()){
                    $detailscat=$histcat->get();
                    foreach($detailscat as  $cat){
                        $data['categorie'][]=['new'=>$cat->displayName,'old'=> $cat->displayNameprecedent];
                    }
                    $nbnotif++;
                }
              ...
              if($photo->exists()){
                    ...
                    foreach($detailsphoto as  $pho){
                        $data['photo'][]=['new'=>$pho->file,'old'=> $pho->fileprecedent];
                    }
                    $nbnotif++;
                }
