...
class UserController extends Controller {
    ...
    public function profilincomplet(Request $request) {
        try {
          ...
          ['path'=>"https://api-wallpost.bforbiz-dev.com/public/icon/complet.svg",'etat'=>'complet',"namedr"=> "2/2 groupes d'étiquettes","status"=>true],
          ['path'=>"https://api-wallpost.bforbiz-dev.com/public/icon/aucune.svg",'etat'=>'aucune',"namedr"=> "1/2 groupes d'étiquettes","status"=>true],
          ['path'=>"https://api-wallpost.bforbiz-dev.com/public/icon/manque.svg",'etat'=>'manque',"namedr"=> "0/2 groupes d'étiquettes","status"=>true]]];
         $franchise_id=$request->header('franchise');
     ...
     $fichestotals =profilincomplete::query()
            ...
            if ($request->drapeaux ==="" || (empty($request->drapeaux) && ($request->drapeaux !=""))){  
                $fiches=$fiche->get();
                $fichestotal=  $fichestotals->first();
            }else{
                foreach($request->drapeaux as $Drapeaux ){
         
                   $fiche->when($Drapeaux['etat']==='manque' &&  $Drapeaux['status']==true,function ($query) use($Drapeaux){
                        $query->where('etat', $Drapeaux['path']);
                        })
                       ->when($Drapeaux['etat']==='complet' &&  $Drapeaux['status']==true,function ($query) use($Drapeaux){
                        $query->where('etat', $Drapeaux['path']);
                            })
                    ->when($Drapeaux['etat']==='aucune' &&  $Drapeaux['status']==true,function ($query) use($Drapeaux){
                        $query->where('etat', $Drapeaux['path']);
                                });
                                $fiches=$fiche->get();     
                                $fichestotals->when($Drapeaux['etat']==='manque' &&  $Drapeaux['status']==true,function ($query) use($Drapeaux){
                                    $query->where('etat', $Drapeaux['path']);
                                    })
                                   ->when($Drapeaux['etat']==='complet' &&  $Drapeaux['status']==true,function ($query) use($Drapeaux){
                                    $query->where('etat', $Drapeaux['path']);
     ...
     if (count($fiches)> 0) {
        ...
        'etat'=>null, 'id'=>null,'locationName'=>null,'total'=>0];
...
public function profilincompletapi(Request $request) {
    try {
        if (!is_numeric($request->header('franchise'))) {
            return response()->json([
                'success' => false,
    ...
    // ->where('states.isVerified','=',1)
         ...
         ->select('fiches.*' )
        // , 'categories.displayName', 'categories.id as categorie_id', 'categories.type')
     ->limit(100)
         ->get();
         ...
         if(isset($request->drapeaux) && !empty($request->drapeaux) && ($request->drapeaux !="")) {
            $tabfiche=array();
            $fichefrist=array();
            $fichestab = Ficheuser::join('fiches', 'fiches.id', '=', 'ficheusers.fiche_id')->where("fiches.franchises_id","=",$request->header('franchise'))->where("user_id","=",Auth()->user()->id)->where('fiches.state', 'LIKE', 'COMPLETED')->get();
            foreach($fiches as $list){
               // $tabfiche[]=$list;
            foreach($request->drapeaux as $Drapeaux ){
             
                if($Drapeaux['etat']==='manque' &&  $Drapeaux['status']==true){
                  
              if((Etiquetgroupe::where('groupe_id', '=', 1)->where('fiche_id',$list['id'])->where('state','=',1)->doesntExist() && Etiquetgroupe::Where('groupe_id', '=', 2)
              ->where('fiche_id',$list['id'])->where('state','=',1)->exists())|| (Etiquetgroupe::where('groupe_id', '=', 2)->where('fiche_id',$list['id'])->where('state','=',1)->doesntExist() && Etiquetgroupe::Where('groupe_id', '=', 1)
              ->where('fiche_id',$list['id'])->where('state','=',1)->exists())){
                
                $fichefrist=$list;
               $tabfiche[]=$list;
               }
                }
                if($Drapeaux['etat']==='complet' &&  $Drapeaux['status']==true){
                    if(Etiquetgroupe::where('groupe_id',1)->where('state','=',1)->where('fiche_id',$list['id'])->exists() &&
                     Etiquetgroupe::where('groupe_id',2)->where('state','=',1)->where('fiche_id',$list['id'])->exists()){
                        $fichefrist=$list;
                $tabfiche[]=$list;
            }
            }
            if($Drapeaux['etat']==='aucune' &&  $Drapeaux['status']==true){
                if (Etiquetgroupe::where('groupe_id','!=',1)->where('fiche_id',$list['id'])->where('state','=',1)->doesntExist() &&
                 Etiquetgroupe::where('groupe_id','!=',2)->where('fiche_id',$list['id'])->where('state','=',1)->doesntExist()){
                    $fichefrist=$list;
                    $tabfiche[]=$list;
                 }
             }
            ...
            foreach($request->drapeaux as $Drapeaux ){
               ...
               $Drapeaux['status']==true?  $status=$Drapeaux['status']: $status=false;
                ...
                $listdrapeux[]=['path'=>"https://api-wallpost.bforbiz-dev.com/public/icon/".$Drapeaux['etat'].".svg",'etat'=>$Drapeaux['etat'],'namedr'=>$Drapeaux['namedr'],"status"=>$status];
            }    
 ...
 if (count($tabfiche) > 0) {
    $i = 0;
    foreach ($tabfiche as $fiche) {

        $pr =Paramater::where('name','profilincomplet')->first();
        $prs= (int)$pr->value;
        !$fiche['address'] ? $prs--: '' ;
        !$fiche['locationName'] ? $prs--: '' ;
        !$fiche['primaryPhone'] ? $prs--: '' ;
        !$fiche['websiteUrl']? $prs--: '' ;
        !$fiche['labels']? $prs--: '' ;
        !$fiche['adwPhone']? $prs--: '' ;
        !$fiche['description']? $prs--: '' ;
        !$fiche['storeCode']? $prs--: '' ;
      Fichehour::where('fiche_id', $fiche['id'])->whereNotNull('close_date')->whereNotNull('open_date')-> whereNull('specialhours_start_date')->whereNull('specialhours_end_date')->doesntExist()? $prs--: '' ;
     
      Fichehour::where('fiche_id', $fiche['id'])->whereNull('open_date')->whereNull('close_date')->whereNotNull('specialhours_start_date') ->whereNotNull('specialhours_end_date')->doesntExist()? $prs--: '' ;
      Service::leftJoin('categories', 'categories.id', '=', 'services.categorie_id')
      ->where('categories.type', 'primaryCategory')->where('categories.fiche_id', $fiche['id'])->doesntExist()? $prs--: '' ;
      Attribute::where('fiche_id', $fiche['id'])->where('valueType','!=','URL')->doesntExist()? $prs--: '' ;
      Photo::where('fiche_id', $fiche['id'])->where('category','PROFILE')->doesntExist()? $prs--: '' ;
      Attribute::where('fiche_id', $fiche['id'])->where('valueType','URL')->doesntExist()? $prs--: '' ;
      Post::join('postfiches', 'postfiches.post_id', '=', 'posts.id')
      ->where('postfiches.fiche_id', $fiche['id'])
      ->where('posts.type','=','Produits')->doesntExist()? $prs--: '' ;
      Morehours::where('fiche_id', $fiche['id'])->doesntExist()? $prs--: '' ;
      Servicearea::where('fiche_id', $fiche['id'])->doesntExist()? $prs--: '' ;
       // $prc = $prs * 100 /8;
        $prc = $prs * 100 /(int)$pr->value;
        // $prc=100-$prc;
         $data[$fiche['id']] = array('id' => $fiche['id'],
         'etat'=>FranchiseController::statefiche($fiche['id']),
     ...
     $totalp = $total * 100 / $i;
     $dates[] = array('profil' => $data, 'totalprofil' => (int) $totalp,'nbcount'=>count($data),"nbfichee"=>count($tabfiche),"fic"=>$fiches);
 } else {
        $data=[
            'etat'=>null, 'id'=>null,'locationName'=>null,'total'=>null];
         $dates = array('profil' => $data, 'totalprofil' => number_format(0, 0, ',', ''),'nbcount'=>count($data));
     }
     return response()->json([
                 'success' => true,
                 'message' => 'List profil utilisateur',
        ...
        try {
         ...
         $fiche_id=$request->fiche_id;
            ...
            ->where('states.isVerified','=',1)
             ...
             ->select('fiches.*', 'categories.displayName', 'categories.id as categorie_id', 'categories.type')->limit(100)
             ->get();
        ...
        foreach ($fiches as $fiche) {
          $fichehours = Fichehour::where('fiche_id', $fiche['id'])
            ...
            $displayNameservice = Service::whereIN('categorie_id', $tabcategoriadds)->doesntExist();
             //$displayNameservice = Service::where('categorie_id', $fiche['categorie_id'])->doesntExist();
            ...
            $hours=0;
                    ...
                    if ($photo) {
                       $iconfiche= Iconfiche::where('code','logo')->first();
                        ...
                        $listfiche[$iconfiche->name][]=['name'=>$iconfiche->name,'icon'=>\Illuminate\Support\Facades\URL::to('/') .'/'.$iconfiche->path,'total'=>$logo];
                    }
                    if (!$fiche['primaryPhone']) {
                        ...
                        $listfiche[$iconfiche->name][]=['name'=>$iconfiche->name,'icon'=>\Illuminate\Support\Facades\URL::to('/') .'/'.$iconfiche->path,'total'=>$tel];
                    ...
                    if (!$fiche['websiteUrl']) {
                        ...
                        $url++;
                        $listfiche[$iconfiche->name][]  =['name'=>$iconfiche->name,'icon'=>\Illuminate\Support\Facades\URL::to('/') .'/'.$iconfiche->path,'total'=>$url];
                    }
                    if ($priserendez) {
                        ...
                        $attribute++;
                        $listfiche[$iconfiche->name][]=['name'=>$iconfiche->name,'icon'=>\Illuminate\Support\Facades\URL::to('/') .'/'.$iconfiche->path,'total'=>$attribute];
                    ...
                    if ($displayNameservice) {
                        ...
                        $service++;
                        $listfiche[$iconfiche->name][]=['name'=>$iconfiche->name,'icon'=>\Illuminate\Support\Facades\URL::to('/') .'/'.$iconfiche->path,'total'=>$service];
                       
                    }
                    if ($specialhours) {
                        $iconfiche= Iconfiche::where('code','specialhours_start_date')->first();
                        $housspec++;
                        $listfiche[$iconfiche->name][]=['name'=>$iconfiche->name,'icon'=>\Illuminate\Support\Facades\URL::to('/') .'/'.$iconfiche->path,'total'=>$housspec];
                    ...
                    if ($fichehours) {
                        ...
                        $hours++;
                        $listfiche[$iconfiche->name][]=['name'=>$iconfiche->name,'icon'=>\Illuminate\Support\Facades\URL::to('/') .'/'.$iconfiche->path,'total'=>$hours];
                    }
                ...
                foreach ($listfiche as $k=>$subArray) {
                  foreach ($subArray as $value) {
                    $sumArray=['total'=>count($subArray),'name'=>$value['name'],'icon'=>$value['icon']];
                  }
